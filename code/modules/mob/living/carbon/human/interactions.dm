...
/mob/living/carbon/human/proc/is_nude()
	return (!wear_suit && !w_uniform && (underwear == 0 || underwear == 12)) ? 1 : 0
...
/mob/living/carbon/human/make_interaction()
	...
	var/hashands = (!H.get_held_item_by_index(GRASP_RIGHT_HAND) && !H.get_held_item_by_index(GRASP_LEFT_HAND)) ? 1 : 0
	var/hashands_p = (!P.get_held_item_by_index(GRASP_RIGHT_HAND) && !P.get_held_item_by_index(GRASP_LEFT_HAND)) ? 1 : 0
	var/mouthfree = !H.get_body_part_coverage(MOUTH)
	var/mouthfree_p = !P.get_body_part_coverage(MOUTH)
	...
	if (hashands)
		...
		if (Adjacent(P))
			dat +=  {"• <A href='?src=\ref[usr];interaction=handshake'>Пожать руку.</A><BR>"}
	...
	if (isnude && usr.loc == H.partner.loc)
		if (haspenis && hashands)
			...
			if (isnude_p)
				...
				if (mouthfree_p && (P.species != "Wryn") && (P.species.name != "Kidan") && (P.species.name != "Diona") && (P.species.name != "Drask"))
	...
	var/datum/browser/popup = new(usr, "mob\ref[src]I", "Interactions", 340, 500)
...
mob/living/carbon/human/proc/cum(mob/living/carbon/human/H as mob, mob/living/carbon/human/P as mob, var/hole = "floor")
	...
	if (H.gender == MALE)
		...
		if (H.multiorgasms < H.potenzia)
			...
			C.blood_DNA[H.dna.unique_enzymes] = H.dna.b_type
			C.virus2 = virus_copylist(H.virus2)
		...
		playsound(loc, "honk/sound/interactions/final_m[rand(1, 5)].ogg", 50, 0)
		...
		if (istype(P.loc, /obj/structure/closet))
			P.visible_message("<B>[H] [message].</B>")
		playsound(loc, "honk/sound/interactions/final_f[rand(1, 3)].ogg", 70, 0)
	...
	switch(hole)
		...
		if("fingering")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					...
					P.moan()
			playsound(loc, "honk/sound/interactions/champ_fingering.ogg", 50, 1)
			H.do_fucking_animation(P)
		...
		if("blowjob")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					P.cum(P, H, "mouth")
			playsound(loc, "honk/sound/interactions/bj[rand(1, 11)].ogg", 50, 1)
			H.do_fucking_animation(P)
			if (prob(P.potenzia))
				H.oxyloss += 3
		...
		if("vaginal")
			...
			if (prob(35))
				switch(P.species.name)
					...
					if("Slime People")
						message = pick("грубо трахает [P].", "предаётс[ya] страстной любви с [P].", "резким движением погружаетс[ya] внутрь [P].", "движетс[ya] внутри [P].", "двигает тазом, засажива[ya] член в [P].", "стонет, навалива[ya]сь на [P].", "сильно прижимаетс[ya] пахом к [P].", "насаживает [P] на свой член.", "чувственно имеет [P].")
						playsound(loc, "honk/sound/interactions/champ[rand(1, 2)].ogg", 50, 1)
			...
			if (istype(P.loc, /obj/structure/closet))
				P.visible_message("<font color=purple>[H] [message]</font>")
				playsound(P.loc.loc, 'sound/effects/clang.ogg', 50, 1)
			H.lust += 10
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					...
					P.moan()
			H.do_fucking_animation(P)
			playsound(loc, "honk/sound/interactions/bang[rand(1, 3)].ogg", 70, 1)
		...
		if("oral")
			...
			H.do_fucking_animation(P)
			playsound(loc, "honk/sound/interactions/oral[rand(1, 2)].ogg", 70, 1)
...
mob/living/carbon/human/proc/moan()
	...
	switch(species.name)
		if ("Human", "Skrell", "Slime People", "Grey", "Plasmaman")
			if (prob(H.lust / H.resistenza * 65))
				...
				if(!istype(loc, /obj/structure/closet))
					playsound(loc, "honk/sound/interactions/moan_[g][moan].ogg", 50, 0)
				else if (g == "f")
					playsound(loc, "honk/sound/interactions/under_moan_f[rand(1, 4)].ogg", 50, 0)
				lastmoan = moan

				if (istype(H.head, /obj/item/clothing/head/kitty)  || istype(H.head, /obj/item/clothing/head/collectable/kitty))
					playsound(loc, "honk/sound/interactions/purr[rand(1, 3)].ogg", 70, 1)
